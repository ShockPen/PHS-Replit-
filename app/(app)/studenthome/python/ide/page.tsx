...
`,
  ...
  const runCode = async () => {
    ...
    setCurrentStep(0)
    setInputBuffer([])
    setOutput([{ text: `Running ${activeFile}...`, type: "system", timestamp: new Date().toLocaleTimeString() }])

    try {
      // Check if this is an interactive program
      if (activeFileContent.includes("input(")) {
        // Start progressive execution
        await executeProgressively(activeFileContent)
      } else {
        // Non-interactive program, run normally
        await executeNonInteractive(activeFileContent)
      }
    } catch (error) {
  ...
  const executeProgressively = async (code: string) => {
    // For the demo, we'll handle the specific case in the default code
    // In a real implementation, you'd need to parse the Python code more intelligently

    // Show initial output
    addToOutput("Hello, World!")

    // Show first question
    addToOutput("What's your name? ")
    setIsWaitingForInput(true)
    setInputPrompt("What's your name? ")
    setCurrentStep(0)
  }

  const executeNonInteractive = async (code: string) => {
    try {
...
captured_output.getvalue()
  ...
  const handleInputSubmit = () => {
    if (!isWaitingForInput || !userInput.trim()) return

    // Add user input to output
    addToOutput(userInput, "input")

    // Add to input buffer
    const newInputBuffer = [...inputBuffer, userInput]
    setInputBuffer(newInputBuffer)

    // Process based on current step
    if (currentStep === 0) {
      // First input (name)
      addToOutput(`Nice to meet you, ${userInput}!`)
      addToOutput("How old are you? ")
      setCurrentStep(1)
      setUserInput("")
      setInputPrompt("How old are you? ")
    } else if (currentStep === 1) {
      // Second input (age)
      try {
        const age = Number.parseInt(userInput, 10)
        if (isNaN(age)) {
          addToOutput("That's not a valid age. Program terminated.", "error")
          setProgramRunning(false)
          setIsWaitingForInput(false)
          return
        }

        addToOutput(`In 10 years, you will be ${age + 10} years old!`)
        addToOutput("--- Program execution completed ---", "system")
        setProgramRunning(false)
        setIsWaitingForInput(false)
      } catch (error) {
        addToOutput("Error processing age. Program terminated.", "error")
        setProgramRunning(false)
        setIsWaitingForInput(false)
      }
      setUserInput("")
            ...
            <div className="space-y-4 flex-shrink-0">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  onClick={() => router.push("/studenthome/python/repo")}
                  disabled={!pyodide}
                >
